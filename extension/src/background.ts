...
async function bootstrap() {
  ...
  worker.onmessage = (event: MessageEvent) => {
    ...
    if (payload.type === "result") {
      const targetTab = payload.tabId as number | undefined;
...
async function logUncertain(entry: Record<string, unknown>) {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type === "score-sentences") {
    ...
    const sentences: Array<string | { text: string; localId?: string }> = Array.isArray(message.sentences)
      ? message.sentences
      : [];
